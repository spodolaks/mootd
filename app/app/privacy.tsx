...
export default function PrivacyScreen() {
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setBusy(true);
            try {
              const report = await apiClient.delete<PurgeReport>('/v1/privacy/self');
              Alert.alert(
                'Account deleted',
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      Alert.alert('Export failed', e instanceof Error ? e.message : 'Please try again later.');
    } finally {
      setExportBusy(false);
    }
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor }]} edges={['top']}>
      ...
      <ScrollView
        ...
        <View style={[styles.section, { backgroundColor: cardBg }]}>
          <Text style={[styles.sectionTitle, { color: textColor }]}>Export your data</Text>
          <Text style={[styles.sectionBody, { color: secondaryText }]}>
            Download every record we hold about you — wardrobe items, outfits, moodboards, and
            account history — as a JSON archive.
          </Text>
          <Pressable
        ...
        <View style={[styles.section, { backgroundColor: cardBg }]}>
          ...
          <Text style={[styles.sectionBody, { color: secondaryText }]}>
            Permanently erase every record about you. This cannot be undone. Type{' '}
            <Text style={{ ...typography.body.semiBold, color: textColor }}>{expectedConfirm}</Text>{' '}
            below to confirm.
          </Text>
          <TextInput
            value={confirm}
            ...
            style={[styles.input, { color: textColor, borderColor: dividerColor }]}
            editable={!busy}
          />
          <Pressable
            style={[
